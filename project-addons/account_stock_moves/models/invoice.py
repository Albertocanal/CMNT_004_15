...
class AccountInvoice(models.Model):
    ...
    def invoice_line_move_line_get(self):
        ...
        if self.type == 'in_invoice':
            for line_data in res:
                ...
                import ipdb
                ipdb.set_trace()
                if i_line.move_line_ids and i_line.purchase_line_id and i_line.product_id.valuation == 'real_time':
                    purchase_line = i_line.purchase_line_id
                    qty = i_line.quantity
                    if i_line.product_id.cost_method in ['average', 'fifo'] and purchase_line.price_unit:
                    ...
                    if len(move_lines) > 1:
                        line_data['create_date'] = i_line.move_line_ids[0]._get_origin_create_date()
                    else:
    ...
    def _cost_diff_move_lines(self, res):
        ...
        for line in res:
            ...
            a = i_line.product_id.product_tmpl_id.get_product_accounts(fiscal_pos=fpos)['stock_input'].id
            import ipdb
            ipdb.set_trace()
            if 'purchase_line' in line and line['purchase_line'].currency_id.id != i_line.invoice_id.currency_id.id:
                line['price_purchase'] = line['purchase_line'].currency_id.with_context(date=line['create_date']).compute(line['price_purchase'], i_line.invoice_id.currency_id, round=True)
            price_subtotal = i_line.price_subtotal
            ipdb.set_trace()
            if 'price_purchase' in line and acc and tools.float_compare(line['price_purchase'], price_subtotal, 2):
    ...
    def action_cancel(self):
        ...
        for i_line in self.invoice_line_ids:
            i_line['price_purchase'] = i_line['purchase_line'].currency_id.with_context(date=line['create_date']).compute(line['price_purchase'], i_line.invoice_id.currency_id, round=True)
            price_subtotal = i_line.price_subtotal
            ipdb.set_trace()
            if 'price_purchase' in line and acc and tools.float_compare(line['price_purchase'], price_subtotal, 2):
                price_diff = price_subtotal - line['price_purchase']
